...
from dtools.primitives.washers import Washer
...
def sampler():
    ...
    width = 15
    offset = 12
    heatset_part_height = 10
    screw_part_height = 10
    screws: list[Screw] = [
        MetricScrews.M2.copy_with(length=8),
        MetricScrews.M3.copy_with(length=8),
        MetricScrews.M4.copy_with(length=10),
    ]
    length = offset * (len(screws) + 0.5)
    heatsert_part = (
        Workplane("XY")
        .box(length, width, heatset_part_height, centered=False)
        .edges("|Z")
        .fillet(4)
    )
    screw_part = (
        Workplane("XY")
        .box(length, width, screw_part_height, centered=False)
        .edges("|Z")
        .fillet(4)
    )

    rev_text = Workplane("XZ").text("v" + _VERSION, 4, distance=1).translate((10, 1, 3))

    screw_part -= rev_text
    heatsert_part -= rev_text

    offset_iter = offset / 2
    for screw in screws:
        hole_pos = (offset_iter, width / 2)
        joint = HeatsertJoint(screw, boss_height=3, heatsert_length="4")

        heatsert_part = joint.apply_female(heatsert_part, "Z>", offset=hole_pos)
        screw_part = joint.apply_male(screw_part, "Z>", offset=hole_pos)
        offset_iter += offset

    heatsert_part = heatsert_part
    ass = cq.Assembly()
    ass.add(heatsert_part)
    ass.add(
        screw_part,
        loc=cq.Location(0, 0, heatsert_part.get_bbox().zmax),
        color=cq.Color("red"),
    )
    show(ass)

    build_path = Path("build/heatsert_join")
    build_path.mkdir(parents=True, exist_ok=True)
    screw_part.export(build_path / "top.stl")
    heatsert_part.export(build_path / "bottom.stl")


def demo():
    from ocp_vscode import show

    cube = Workplane("XY").box(20, 20, 20)
    cube2 = Workplane("XY").box(20, 20, 20)
    cube3 = Workplane("XY").box(20, 20, 20)
    cube4 = Workplane("XY").box(20, 20, 20)

    joint = HeatsertJoint(
        MetricScrews.M2.copy_with(length=10), boss_height=3, heatsert_length="4"
    )
    cube = joint.apply_male(cube, "Z>", (5, 0))
    cube2 = joint.apply_male(cube2, "Z<")

    cube3 = joint.apply_female(cube3, "Z>", (5, 0))
    cube4 = joint.apply_female(cube4, "Z<")

    ass = cq.Assembly()
    ass.add(cube, color=cq.Color("blue"))
    ass.add(cube2, loc=cq.Location(25, 0, 0), color=cq.Color("red"))
    ass.add(cube3, loc=cq.Location(0, 0, -25), color=cq.Color("blue"))
    ass.add(cube4, loc=cq.Location(25, 0, 25), color=cq.Color("red"))
    show(ass, transparent=True)
