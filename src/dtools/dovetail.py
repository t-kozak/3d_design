...
def create_dovetail_key(workplane: Workplane, cfg: DovetailJointConfig) -> Workplane:
    # Geometry parameters
...
def create_dovetail_keyway(workplane: Workplane, cfg: DovetailJointConfig) -> Workplane:
...
def sample():
    ...
    cfg = DovetailJointConfig(key_width=10.0, key_depth=10.0, amount=2, spacing=10.0)
    key = create_dovetail_key(Workplane("XY"), cfg)
    keyway = create_dovetail_keyway(Workplane("XY"), cfg)

    ass = cq.Assembly()
    ass.add(key, name="key")
    ass.add(keyway, name="keyway", loc=cq.Location(Vector(0, 10, 0)))
