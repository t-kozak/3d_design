...
from cadquery import Compound, Solid
...
def imperial_str_to_float(measure: str) -> float:
    ...
    if re.match(r"\d+", measure):
        return float(measure) * IN
    frac = re.match(r"(\d+) */(\d+) *", measure)
    if frac:
        nom = int(frac.group(1))
        den = int(frac.group(2))
        return nom / den * IN
    mixed_frac = re.match(r"(\d+) *(\d+) */ *(\d+)", measure)
    if mixed_frac:
        return IN * (
            float(mixed_frac.group(1)) + float(mixed_frac.group(2)) / float(mixed_frac.group(3))
        )
...
class Thread(Solid):
    ...
    def fade_helix(
        ...
        if self.external:
            radius = (
                self.apex_radius - sin(t * pi / 2) * self.tooth_height if apex else self.root_radius
            )
        else:
            radius = (
                self.apex_radius + sin(t * pi / 2) * self.tooth_height if apex else self.root_radius
    ...
    def __init__(
        ...
        if not simple:
            ...
            cylindrical_thread_length = self.length + self.pitch * (1 - 1 * number_faded_ends)
    ...
    def make_thread_with_faded_ends(
        ...
        thread_faces = [
            cq.Face.cast(f.translate((0, 0, cylindrical_thread_displacement))) for f in thread_faces
        ]
        end_faces = [
            cq.Face.cast(f.translate((0, 0, cylindrical_thread_displacement))) for f in end_faces
        ]
        cylindrical_thread_angle = (
            (360 if self.right_hand else -360) * cylindrical_thread_length / self.pitch
        )
        fade_faces, _ = self.make_thread_faces(self.pitch / 4, fade_helix=True)
        if not self.right_hand:
            fade_faces = [cq.Face.cast(f.mirror("XZ")) for f in fade_faces]
        ...
        if self.end_finishes[0] == "fade":
            ...
            if self.apex_offset != 0:
                fade_faces_bottom, _ = self.make_thread_faces(
                    self.pitch / 4, fade_helix=True, asymmetric_flip=True
                )
                if not self.right_hand:
                    fade_faces_bottom = [cq.Face.cast(f.mirror("XZ")) for f in fade_faces_bottom]
            else:
                fade_faces_bottom = fade_faces
            fade_faces_bottom = [
                cq.Face.cast(f.mirror("XZ").mirror("XY").translate(cq.Vector(0, 0, self.pitch / 2)))
        ...
        if number_faded_ends == 2:
            thread_shell = cq.Shell.makeShell(thread_faces + fade_faces_bottom + fade_faces_top)  # type: ignore
        elif self.end_finishes[0] == "fade":
            thread_shell = cq.Shell.makeShell(thread_faces + fade_faces_bottom + [end_faces[1]])  # type: ignore
        else:
            thread_shell = cq.Shell.makeShell(thread_faces + fade_faces_top + [end_faces[0]])  # type: ignore
        ...
        if self.end_finishes.count("square") != 0:
            ...
            for i in range(2):
                if self.end_finishes[i] == "square":
                    squared = cq_object.cut(cutter.translate(cq.Vector(0, 0, 2 * i * self.length)))
        return squared

    def chamfer_ends(self, cq_object: Solid | cq.Shape):
        ...
        if self.end_finishes.count("chamfer") != 0:
            ...
            chamfered = cq_object.intersect(cutter.val())  # type: ignore
        return chamfered
    ...
    def make_thread_faces(
        ...
        apex_helix_wires = [
            cq.Workplane("XY")
            .parametricCurve(lambda t: self.fade_helix(t, apex=True, vertical_displacement=0))
            .val()
            .translate((0, 0, i * self.apex_width + local_apex_offset))  # type: ignore
            if fade_helix
        ...
        for i in [-0.5, 0.5]:
            if fade_helix:
                itm = (
                    cq.Workplane("XY")
                    .parametricCurve(
                        lambda t, idx=i: self.fade_helix(
                            t,
                            apex=False,
                            vertical_displacement=idx * (self.root_width - self.apex_width),
                        )
                    )
                    .val()
                    .translate((0, 0, i * self.root_width))  # type: ignore
                )
            else:
                itm = cq.Wire.makeHelix(
                    pitch=self.pitch,
                    height=length,
        ...
        end_cap_wires = [
            cq.Wire.makePolygon(
                [
                    apex_helix_wires[0].positionAt(i),  # type: ignore
                    apex_helix_wires[1].positionAt(i),  # type: ignore
                    root_helix_wires[1].positionAt(i),  # type: ignore
                    root_helix_wires[0].positionAt(i),  # type: ignore
                    apex_helix_wires[0].positionAt(i),  # type: ignore
                ]
            )
            for i in end_caps
        ...
        thread_faces = [
            cq.Face.makeRuledSurface(apex_helix_wires[0], apex_helix_wires[1]),  # type: ignore
            cq.Face.makeRuledSurface(apex_helix_wires[1], root_helix_wires[1]),  # type: ignore
            cq.Face.makeRuledSurface(root_helix_wires[1], root_helix_wires[0]),  # type: ignore
            cq.Face.makeRuledSurface(root_helix_wires[0], apex_helix_wires[0]),  # type: ignore
        ]
...
class AcmeThread(TrapezoidalThread):
    ...
    def parse_size(cls, size: str) -> Tuple[float, float]:
        """Convert the provided size into a tuple of diameter and pitch"""
        if size not in AcmeThread.acme_pitch.keys():
            raise ValueError(f"size invalid, must be one of {AcmeThread.acme_pitch.keys()}")
...
class PlasticBottleThread(Solid):
    ...
    finish_data = {
        ...
        110: [1.125,[28]],
        200: [1.5,[24.28]],
        400: [1.0,[18,20,22,24,28,30,33,35,38,40,43,45,48,51,53,58,60,63,66,70,75,77,83,89,100,110,120]],  # noqa: E501
    ...
    def __init__(
        ...
        if self.diameter not in PlasticBottleThread.finish_data[self.finish][1]:
            raise ValueError(
                f"diameter ({self.diameter}) invalid, must be one"
                f" of {PlasticBottleThread.finish_data[self.finish][1]}"
            )
        (diameter_max, diameter_min, self.tpi) = self.THREAD_DIMENSIONS[self.diameter]
        if self.style == "L":
            (
                self.root_width,
                thread_height,
            ) = self.L_STYLE_THREAD_DIMENSIONS[self.tpi]
        else:
            (
                self.root_width,
                thread_height,
            ) = self.M_STYLE_THREAD_DIMENSIONS[self.tpi]
        if self.external:
            self.apex_radius = diameter_min / 2 - manufacturingCompensation
            self.root_radius = diameter_min / 2 - thread_height - manufacturingCompensation
        else:
            self.root_radius = diameter_max / 2 + manufacturingCompensation
            self.apex_radius = diameter_max / 2 - thread_height + manufacturingCompensation
        self.thread_angles = self.THREAD_ANGLES[self.style + str(self.finish)]
        ...
        self.length = (PlasticBottleThread.finish_data[self.finish][0] + 0.75) * self.pitch
        cq_object = Thread(
            apex_radius=self.apex_radius,
